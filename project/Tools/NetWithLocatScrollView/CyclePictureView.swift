...
class CyclePictureView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, PageControlAlimentProtocol, EndlessCycleProtocol {
    ...
    internal var imageBox: ImageBox?
    /// 开启无限滚动模式后,真实的cell数量
    ...
    internal let cellID: String = "CyclePictureCell"
...
extension CyclePictureView {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(CGFloat(Int(offsetIndex) % self.imageBox!.imageArray.count) + 0.5)
...
extension CyclePictureView {
    ...
    @objc(collectionView:cellForItemAtIndexPath:) func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath as IndexPath) as! CyclePictureCell
    ...
    @objc(collectionView:didSelectItemAtIndexPath:) func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        self.delegate?.cyclePictureView(cyclePictureView: self, didSelectItemAtIndexPath: NSIndexPath(item: indexPath.item % self.imageBox!.imageArray.count, section: indexPath.section))
    }
