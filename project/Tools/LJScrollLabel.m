...

{
    ...
    CGFloat _textWidth;
    LJLabelScrollMode _currentScrollModel;
    LJLabelScrollMoveDirection _currentMoveDirection;
...
-(void)startScroll{
    ...
    switch (_currentScrollModel) {
        ...
        case LJLabelScrollContinuous:
        {
            if (_textWidth > self.frame.size.width) {
                if (_currentMoveDirection == LJLabelScrollMoveLeft) {
        ...
        case LJLabelScrollIntermittent:
        {
            if (_currentMoveDirection == LJLabelScrollMoveLeft) {
                [self creatLabel1WithFrame:CGRectMake(0, 0, _textWidth, self.frame.size.height)];
            }else{
...
- (void)contentMove {
    ...
    if (_textWidth < self.frame.size.width) {//如果字符串长度小于控件宽度，不滚动
        ...
        if (_currentScrollModel == LJLabelScrollWandering) {//往返模式除外
            [self moveWandering];
        }
        return;
    }
